...
public class Book {
    ...
    public static String capitalize(String word) {
        String [] allChar = word.toLowerCase().split("");
        String newWord= allChar[0].toUpperCase();
    ...
    public static Boolean isTitleWord(String word) {
        List<String> lst = Arrays.asList("and", "but", "for", "nor", "or", "so", "yet",
                "at", "by", "in", "into", "near", "of", "on", "to", "than", "via", "a", "an", "the");

        for (int i = 0; i < lst.size(); i++) {
            if (lst.get(i).equals(word.toLowerCase())) {
    ...
    public Book(int id, String author, String title, int pages ) {
        ...
        public void setTitle(String title) {
            if(title.equals("")){
                ...
                this.title = allWords[0].substring(0,1).toUpperCase()+allWords[0].substring(1).toLowerCase();
                for (int i = 1; i <allWords.length ; i++) {

                    if(!Book.isTitleWord(allWords[i])){
                        this.title +=" "+allWords[i].substring(0,1).toUpperCase()+allWords[i].substring(1).toLowerCase();

                    }else {

                        this.title+=" "+allWords[i].toLowerCase();
                    }
